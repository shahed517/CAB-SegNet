...
parser.add_argument('--train_dir', type=str, required = True, default='MonuSeg dataset/train_folder')
parser.add_argument('--val_dir', type=str, required = True, default='MonuSeg dataset/val_folder')
parser.add_argument('--ckpt_dir', type=str, required = True, default='Home/ckpt')
...
def train_fn_AMP(loader, model, optimizer, loss1, loss2, loss3, scaler):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = ETA*l1 + (1-ETA)*l2  + LAMBDA*l3
        # backward
...
def main():
    ...
    loss1 = TverskyLoss(alpha=ALPHA, beta=BETA).to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        # train_fn_AMP handles only 1 epoch
