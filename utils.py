...
def save_checkpoint(filename, state, epoch):
    ...
    filename = filename + "/best_weights.pth.tar"
...
def produce_output_masks(test_loader, model, epoch, device="cuda", BATCH_SIZE = 1, save = True):
    model.eval()
    path = f'./predicted_outputs_ep{epoch}_/'
...
#                 cv2.imwrite(os.path.join(path, f'{k}_back.png'), pred_3class[b, :, :, 1]*255)
#                 cv2.imwrite(os.path.join(path, f'{k}_edge.png'), pred_3class[b, :, :, 2]*255)
            k += 1         
    model.train()